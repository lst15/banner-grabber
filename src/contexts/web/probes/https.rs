...
use crate::core::model::{Config, Target, TlsInfo};
        ...
        if let Some(content_length) = parse_content_length(&result.bytes) {
            ...
            if available == 0 || missing == 0 {
                if missing > 0 {
                    ...
                    result.reason = crate::core::model::ReadStopReason::SizeLimit;
                }
            ...
            while read < expected {
                ...
                {
                    Ok(Ok(0)) => {
                        result.reason = crate::core::model::ReadStopReason::ConnectionClosed;
                        break;
                    }
                    Ok(Ok(n)) => {
                        read += n;
                    ...
                    Err(_) => {
                        result.reason = crate::core::model::ReadStopReason::Timeout;
                        break;
                    }
                }
            }
