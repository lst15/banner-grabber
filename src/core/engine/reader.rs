use crate::core::model::{Banner, ReadStopReason, TlsInfo};
...
impl BannerReader {
    ...
    pub fn render(&self, result: ReadResult) -> Banner {
        let raw_hex = crate::core::util::hex::to_hex(&result.bytes);
        let printable = crate::core::util::sanitize_text(&result.bytes);
        Banner {
            raw_hex,
            printable,
