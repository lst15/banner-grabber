use crate::contexts::registry::{
    client_for_target, probe_for_target, udp_client_for_target, ClientRequest, ProbeRequest,
};
use crate::core::model::{
    Config, Diagnostics, Fingerprint, Protocol, ReadStopReason, ScanMode, ScanOutcome, Status,
    TcpMeta,
};
use crate::core::util::{now_iso8601, now_millis};
...
pub trait TargetProcessor: Send + Sync {
    async fn process_target(
        &self,
        target: crate::core::model::Target,
...
impl TargetProcessor for DefaultProcessor {
    async fn process_target(
        ...
        let (webdriver_body, diagnostics) = if config.webdriver {
            ...
            {
                match crate::contexts::web::webdriver::fetch_rendered_body(
                    &target,
                    &config.protocol,
            ...
            #[cfg(not(feature = "webdriver"))]
            {
                (
                    ...
                    Some(Diagnostics {
                        stage: "webdriver".into(),
                        message: "webdriver feature disabled".into(),
                    }),
                )
            }
        } else {
...
fn adjusted_connect_timeout(config: &Config, target: &crate::core::model::Target) -> Duration {
...
mod tests {
    use super::*;
    use crate::core::model::{OutputConfig, OutputFormat, Protocol, Target, TargetSpec};
...
async fn scan_technologies(
    target: &crate::core::model::Target,
    protocol: &Protocol,
) -> Option<crate::core::model::TechnologyScan> {
    let url = format!(
        "{}://{}:{}",
    ...
    let scan_time_seconds = analysis
        ...
        .unwrap_or_default();
    let mut list: Vec<crate::core::model::TechnologyEntry> = match analysis.result {
        Ok(techs) => techs
            .into_iter()
            .map(|tech| crate::core::model::TechnologyEntry {
    ...
    Some(crate::core::model::TechnologyScan {
        scan_time_seconds,
        list,
    })
}
...
#[cfg(not(feature = "web"))]
async fn scan_technologies(
    _target: &crate::core::model::Target,
    _protocol: &Protocol,
) -> Option<crate::core::model::TechnologyScan> {
    None
}
